    ...
    <div class="relative overflow-x-auto pt-10 pb-10">
        <div class="ml-auto text-left mb-5">
            <a class="rounded-md bg-slate-800 text-white border border-1 hover:bg-slate-600 hover:text-white p-3 "
                href="{{ route('videos.create') }}">Add new video</a>
        </div>
        <div class="grid grid-cols-2 md:grid-cols-3 gap-4 my-10">
            @foreach ($videos as $video)
                <div
                    ...
                    <div class="p-5">
                        <a href="{{$video->link}}" class="mb-3 font-normal text-gray-700 dark:text-gray-400 hover:underline"><b>Link :</b> {{$video->link}}</a>
                        <p class="mb-3 mt-3 font-normal text-gray-700 dark:text-gray-400"><b>Created at :</b> {{$video->created_at->toDateString()}}</p>
                        @if (!$video->updated_at == null)
                        <p class="mb-3 mt-3 font-normal text-gray-700 dark:text-gray-400"><b>Updated at :</b> {{$video->updated_at->toDateString()}}</p>
                        @endif
                        ...
                        <form action="{{ route('videos.destroy', $video->id) }}" method="POST" class="inline mt-3">
                            @csrf
                            @method('DELETE')
                        <button type="submit"
